...
function run {
    ...
    mkdir -p ${results_dir}/User${i}/${size_of_forest}trees/Testing
		for (( j=0; j<$number_of_iterations; j+=1 )); do
			...
			$gringo $test_pars $data ${ASP_dir}/User${i}/Training/examples${j}.gringo \
				${ASP_dir}/User${i}/Training/number_of_strict_examples${j}.gringo \
				$outcomes ${ASP_dir}/$rules_test_ASP ${results_dir}/User${i}/${size_of_forest}trees/Training/trees${j}.gringo | \
				$clasp $time_pars > \
				${results_dir}/User${i}/${size_of_forest}trees/Training/res${j}.txt 2>&1
	
			# Testing
			$gringo $test_pars $data ${ASP_dir}/User${i}/Testing/examples${j}.gringo \
				${ASP_dir}/User${i}/Testing/number_of_strict_examples${j}.gringo \
				$outcomes ${ASP_dir}/$rules_test_ASP ${results_dir}/User${i}/${size_of_forest}trees/Training/trees${j}.gringo | \
				$clasp $time_pars > \
				${results_dir}/User${i}/${size_of_forest}trees/Testing/res${j}.txt 2>&1
		done
	done
}
...
function trainForest {
  ...
  cat /dev/null > ${results_dir}/User$1/${size_of_forest}trees/Training/trees$2.gringo
	...
	I=0
	less ${results_dir}/User$1/${size_of_forest}trees/Training/trees_res$2.gringo | while read -r line ; do
	  changedLine=${line//(/($I,}
	  line=${changedLine//)/).}
	  echo $line >> ${results_dir}/User$1/${size_of_forest}trees/Training/trees$2.gringo
	
	  I=$((I+1))
	done
	rm ${results_dir}/User$1/${size_of_forest}trees/Training/trees_res$2.gringo
}
